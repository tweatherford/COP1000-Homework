'''
Assignment#10: Rainfall Version - Sorts data, searches array, and outputs to file and adds stats.
...
sortStatus = False
outputFile = open("Weatherford_Timothy_A10_Python_Rainfall_Output.txt", "w")
...
def makeStats(targetCount,outputFile):
    global rainArray
    length = len(rainArray)
    average = round(sum(rainArray)/len(rainArray),2)
    tempArray = []
    writeHeader("The highest rain in the list is: " + str(rainArray[0]) + "\n")
    writeHeader("The lowest rain in the list is: " + str(rainArray[-1]) + "\n")
    writeHeader("The median rain in the list is: " + str(rainArray[length//2]) + "\n")
    writeHeader("The average rain is: : " + str(average) + "\n")
    writeHeader("Rain data between 1 and 10:")

    for rain in rainArray:
        if rain >= 1 and rain <= 10:
            tempArray.append(rain)

    for temp in tempArray:
        outputFile.write("Rain: " + str(temp) + "\n")

    writeHeader("Number of rain data points between 1 and 10: " + str(len(tempArray)))
